...
bool SceneGame::init(bool red)
{
   ...
   //创建悔棋按钮
    ...
    //从infmation.plist中读取背景音乐名  
    const CCString* voice = dict->valueForKey("voice");    
    sprintf(Voice, "%s", voice->getCString());  

    //从Name.plist中读取返回名  
    const CCString* r = dict->valueForKey("return");    
    sprintf(Return, "%s", r->getCString());  
   ...
   //设置文字的位置  
   label->setPosition(ccp(winSize.width/2 + 110, winSize.height/2 - 120));  
